      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-5 items-start sm:items-center border-b border-slate-200 dark:border-slate-800 pb-5">
              <div className="h-24 w-24 rounded-xl overflow-hidden bg-slate-100 dark:bg-background border border-slate-200 dark:border-slate-850 flex items-center justify-center flex-shrink-0 shadow-inner">
                {item.image_url ? (
                  <ImageLightbox src={item.image_url} alt={item.name} triggerClassName="h-full w-full">
                    <img src={item.image_url} alt={item.name} className="h-full w-full object-cover" />
                  </ImageLightbox>
                ) : (
                  <ImageIcon className="h-10 w-10 text-slate-400 dark:text-slate-650" />
                )}
              </div>
              <div className="space-y-1">
                <span className="font-mono text-sm font-bold text-amber-600 dark:text-amber-500 tracking-wider bg-amber-500/10 border border-amber-500/20 px-2.5 py-1 rounded-md">
                  {item.item_code}
                </span>
                <h2 className="text-xl font-bold text-slate-900 dark:text-white leading-snug mt-2">{item.name}</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-900/40 p-3 rounded-lg border border-slate-200 dark:border-slate-850">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{item.category?.name || '-'}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-900/40 p-3 rounded-lg border border-slate-200 dark:border-slate-850">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{item.supplier?.name || '-'}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-900/40 p-3 rounded-lg border border-slate-200 dark:border-slate-850">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{item.uom?.name || 'PCS'}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-900/40 p-3 rounded-lg border border-slate-200 dark:border-slate-850">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{item.minimum_stock} PCS</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <h4 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                <Info className="h-4 w-4 text-slate-400 dark:text-slate-500" /> Deskripsi Barang
              </h4>
              <div className="bg-slate-50 dark:bg-slate-900/30 border border-slate-200 dark:border-slate-850 rounded-lg p-4 min-h-[80px] text-sm text-slate-700 dark:text-slate-300 leading-relaxed">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-4">
            <div className="border-b border-slate-200 dark:border-slate-800 pb-3 flex justify-between items-center">
              <h3 className="font-semibold text-lg text-slate-900 dark:text-white">Stok di Gudang</h3>
                ...
                visibleBranches.map((branch) => {
                  ...
                  return (
                    <div 
                      ...
                      className={`flex items-center justify-between p-4 rounded-lg border bg-slate-50 dark:bg-slate-900/30 shadow-sm transition-all ${
                        isLow 
                    ...
                    >
                      <div className="min-w-0 pr-3">
                        <p className="text-sm font-bold text-slate-900 dark:text-white truncate">{branch.name}</p>
                        <p className="text-[10px] text-slate-500 font-medium truncate mt-0.5">{branch.location || '-'}</p>
                      </div>
                      
                      <div className="flex flex-col items-end flex-shrink-0">
                        <span className="font-mono text-base font-bold text-slate-900 dark:text-white">
                          {qty} <span className="text-xs font-normal text-slate-500 dark:text-slate-400">{item.uom?.name || 'PCS'}</span>
                        </span>
                        
                        {isLow ? (
                          <span className="text-[10px] font-semibold text-red-600 dark:text-red-400 flex items-center gap-1 mt-1 bg-red-500/10 border border-red-500/20 px-1.5 py-0.5 rounded">
                            <AlertTriangle className="h-3 w-3" /> Stok Rendah
                          </span>
                        ) : (
                          <span className="text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 flex items-center gap-1 mt-1 bg-emerald-500/10 border border-emerald-500/20 px-1.5 py-0.5 rounded">
          ...
          {!isStaff && (
            <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-3">
              <h4 className="text-sm font-semibold text-slate-900 dark:text-slate-350">Laporan Transaksi</h4>
