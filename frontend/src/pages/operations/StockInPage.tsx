  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-4 min-h-[300px]">
            ...
            {items.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center text-slate-500">
                <Archive className="h-12 w-12 mb-3 text-slate-400 dark:text-slate-700" />
                ...
                <AnimatePresence initial={false}>
                  {items.map((item) => (
                    <motion.div
                      ...
                      className="overflow-hidden border border-slate-200 dark:border-border bg-slate-50 dark:bg-slate-900/40 rounded-xl p-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 mb-3"
                    >
                      {/* Item Info */}
                      <div className="flex items-center gap-3">
        ...
        <div className="space-y-6">
          <form onSubmit={handlePreSave} className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="branch_select" className="text-slate-700 dark:text-slate-300">Gudang Tujuan</Label>
              {isSuperAdmin ? (
                <select
                  id="branch_select"
                  name="branch_id"
            ...
            {/* Reference No */}
            <div className="space-y-2">
              <Label htmlFor="reference_no" className="text-slate-700 dark:text-slate-300">No. Referensi (Opsional)</Label>
              <Input
                id="reference_no"
                name="reference_no"
                type="text"
            ...
            {/* Supplier Invoice No */}
            <div className="space-y-2">
              <Label htmlFor="supplier_invoice_no" className="text-slate-700 dark:text-slate-300">No. Invoice Supplier (Opsional)</Label>
              <Input
            ...
            {/* Transaction Date */}
            <div className="space-y-2">
              <Label htmlFor="transaction_date" className="text-slate-700 dark:text-slate-300">Tanggal Transaksi (Opsional)</Label>
              <div className="relative flex items-center">
                ...
                <Input
                  ...
                  className="pl-10 bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-850 text-slate-900 dark:text-white rounded-lg focus-visible:ring-primary"
                />
              </div>
            ...
            {/* Notes */}
            <div className="space-y-2">
              <Label htmlFor="notes" className="text-slate-700 dark:text-slate-300">Catatan Tambahan (Opsional)</Label>
              <textarea
                id="notes"
                name="notes"
            ...
            {formError && (
              <div className="text-sm bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 p-3 rounded-lg">
                {formError}
              </div>
            )}

            {successMsg && (
              <div className="text-sm bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 p-3 rounded-lg">
                {successMsg}
              </div>
            )}
            ...
            <div className="pt-2">
              <motion.div whileTap={{ scale: 0.97 }}>
                <Button
                  ...
                  className="w-full bg-gradient-to-r from-primary to-primary/80 hover:opacity-90 border-0 text-slate-950 font-bold rounded-xl min-h-[44px] shadow-md flex items-center justify-center gap-2"
                >
