    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex-shrink-0">
          <Button
            ...
            className="border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-900 text-slate-700 dark:text-slate-300 flex items-center gap-2 rounded-lg min-h-[40px]"
          >
            {downloadingPdf ? (
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-4">
            <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-800 pb-3">
              <h3 className="font-semibold text-lg text-slate-900 dark:text-white">Status Logistik</h3>
              {getStatusBadge(transfer.status)}
            </div>

            {/* Routing indicator */}
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 bg-slate-50 dark:bg-slate-950 p-4 rounded-lg border border-slate-200 dark:border-slate-850">
              <div className="min-w-0">
                <span className="text-[10px] uppercase font-bold text-slate-500">Gudang Asal</span>
                <p className="font-bold text-slate-900 dark:text-white text-base truncate">{getBranchName(transfer.source_branch_id)}</p>
              </div>
              ...
              <div className="min-w-0">
                ...
                <p className="font-bold text-slate-900 dark:text-white text-base truncate">{getBranchName(transfer.dest_branch_id)}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-slate-700 dark:text-slate-300">
              <div className="flex items-start gap-2">
                <FileText className="h-4 w-4 text-slate-400 dark:text-slate-500 mt-0.5 flex-shrink-0" />
                <div>
                  <span className="font-semibold text-slate-900 dark:text-white block">Catatan Pengirim:</span>
                  <span className="text-slate-600 dark:text-slate-400">{transfer.notes || 'Tidak ada catatan'}</span>
                </div>
              </div>
              {transfer.status === 'received' && (
                <div className="flex items-start gap-2">
                  <CheckCircle2 className="h-4 w-4 text-slate-400 dark:text-slate-500 mt-0.5 flex-shrink-0" />
                  <div>
                    <span className="font-semibold text-slate-900 dark:text-white block">Catatan Penerima:</span>
                    <span className="text-slate-600 dark:text-slate-400">{transfer.received_notes || 'Tidak ada catatan'}</span>
                  </div>
                </div>
              )}
              {transfer.status === 'cancelled' && (
                <div className="flex items-start gap-2 col-span-1 sm:col-span-2 border-t border-red-500/10 pt-3">
                  ...
                  <div>
                    <span className="font-semibold text-red-600 dark:text-red-400 block">Alasan Pembatalan:</span>
                    <span className="text-slate-700 dark:text-slate-300">{transfer.cancellation_reason || '-'}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="space-y-3">
              {transfer.lines.map((line) => {
                ...
                return (
                  <div 
                    key={line.line_id} 
                    className={`border rounded-lg p-4 bg-slate-50 dark:bg-slate-900/30 flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${
                      hasVariance ? 'border-red-500/30 bg-red-500/5' : 'border-slate-200 dark:border-slate-850'
                    }`}
                  >
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">{line.item_name || 'Komponen'}</p>
                      <div className="flex flex-wrap items-center gap-2 mt-1">
                        <span className="text-[10px] font-mono px-1.5 py-0.2 rounded bg-slate-100 dark:bg-slate-850 border border-slate-200 dark:border-slate-750 text-amber-600 dark:text-amber-500">
                          {line.item_code}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-6 justify-between sm:justify-end">
                      <div className="text-right">
                        ...
                        <p className="text-sm font-bold text-slate-900 dark:text-white font-mono">{line.sent_quantity} pcs</p>
                      </div>
                      ...
                      {transfer.status === 'received' && (
                        <>
                          <div className="text-right">
                            ...
                            <p className="text-sm font-bold text-slate-900 dark:text-white font-mono">{line.received_quantity ?? 0} pcs</p>
                          </div>
                    ...
                    {hasVariance && (
                      <div className="w-full sm:hidden border-t border-red-500/10 pt-2 mt-1 text-xs text-red-500 dark:text-red-400">
                ...
                {canCancel && (
                  <motion.div whileTap={{ scale: 0.97 }}>
                    <Button
                      ...
                      className="w-full text-red-500 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 hover:bg-red-500/10 rounded-xl min-h-[44px] border border-red-500/20"
                    >
                      Batalkan Transfer
                    </Button>
                  </motion.div>
          ...
          <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-4">
            ...
            <div className="relative border-l border-slate-200 dark:border-slate-800 pl-4 ml-2 space-y-5">
              ...
              <div className="relative">
                <div className="absolute -left-[21px] mt-0.5 h-3 w-3 rounded-full bg-blue-500 border border-white dark:border-slate-950" />
                <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Mutasi Dibuat</p>
                <p className="text-xs text-slate-900 dark:text-white font-medium">{formatDate(transfer.created_at)}</p>
              ...
              {transfer.shipped_at && (
                <div className="relative">
                  <div className="absolute -left-[21px] mt-0.5 h-3 w-3 rounded-full bg-amber-500 border border-white dark:border-slate-950" />
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Mutasi Dikirim</p>
                  <p className="text-xs text-slate-900 dark:text-white font-medium">{formatDate(transfer.shipped_at)}</p>
              ...
              {transfer.received_at && (
                <div className="relative">
                  <div className="absolute -left-[21px] mt-0.5 h-3 w-3 rounded-full bg-emerald-500 border border-white dark:border-slate-950" />
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Mutasi Diterima</p>
                  <p className="text-xs text-slate-900 dark:text-white font-medium">{formatDate(transfer.received_at)}</p>
              ...
              {transfer.cancelled_at && (
                <div className="relative">
                  <div className="absolute -left-[21px] mt-0.5 h-3 w-3 rounded-full bg-red-500 border border-white dark:border-slate-950" />
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Mutasi Dibatalkan</p>
                  <p className="text-xs text-slate-900 dark:text-white font-medium">{formatDate(transfer.cancelled_at)}</p>
      ...
      <Dialog open={cancelOpen} onOpenChange={setCancelOpen}>
        <DialogContent className="sm:max-w-[425px] bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white shadow-2xl rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-slate-900 dark:text-white">Batalkan Mutasi</DialogTitle>
          ...
          <form onSubmit={handleCancelSubmit} className="space-y-4 pt-2">
            <div className="space-y-2">
              <Label htmlFor="cancellation_reason" className="text-slate-700 dark:text-slate-300">Alasan Pembatalan</Label>
              <textarea
            ...
            {actionError && (
              <div className="text-xs bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 p-2.5 rounded-lg">
                {actionError}
              </div>
            )}
            ...
            <DialogFooter className="gap-2 sm:gap-0">
              <Button
                ...
                className="border border-slate-200 dark:border-slate-850 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white rounded-lg"
              >
                Kembali
              </Button>
              <Button
                type="submit"
                className="bg-red-500 hover:bg-red-600 border-0 text-white font-semibold rounded-lg"
              >
                Konfirmasi Batal
              </Button>
            </DialogFooter>
