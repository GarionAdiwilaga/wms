...
**Decision:** Dashboard metrics must consume `ReportService` aggregations. Dashboards are not allowed to implement separate aggregation logic. `ReportService` is the single source of truth for all operational metrics.
**Business Rule:** The UI must display the exact same numbers as the CSV/XLSX exports.
**Reason:** Prevents discrepancies where a dashboard says "12 Low Stock" but the report says "11 Low Stock". Reports first. Dashboards second.

### Stock Opname Reference Type
**Decision:** Ledger transactions generated by Stock Opname will use a new `reference_type = 'opname'`.
**Reason:** Cleaner reporting and easier filtering/auditing distinct from system migrations or manual adjustments.
