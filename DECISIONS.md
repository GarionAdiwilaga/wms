...
### Stock Opname Snapshots
**Decision:** Stock Opname lines must explicitly store `system_quantity`, `physical_quantity`, and `variance`.
**Business Rule:** Even though variance is derivable, it must be stored explicitly.
**Reason:** Preserves historical context and prevents ambiguity if ledger stock levels change later.
